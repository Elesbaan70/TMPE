...
namespace TrafficManager.Manager.Impl {
    partial class LaneArrowManager {

        private class LegacyLaneArrowManager {

            private readonly LegacyFlags Flags = new LegacyFlags();
            ...
            public void HandleInvalidSegment(ref ExtSegment seg) {
            ...
            private class LegacyFlags {

                public readonly uint lfr = (uint)NetLane.Flags.LeftForwardRight;


                /// <summary>
                /// For each lane: Defines the lane arrows which are set
                /// </summary>
                private LaneArrows?[] laneArrowFlags;

                /// <summary>
                /// For each lane: Defines the lane arrows which are set in highway rule mode (they are not saved)
                /// </summary>
                private LaneArrows?[] highwayLaneArrowFlags;
                ...
                public void ResetSegmentArrowFlags(ushort segmentId) {
                    if (segmentId <= 0) {
                        return;
                    }
            ...
            Log._Debug($"Flags.resetSegmentArrowFlags: Resetting lane arrows of segment {segmentId}.");
                ...
                public bool ResetLaneArrowFlags(uint laneId) {
#if DEBUGFLAGS
            Log._Debug($"Flags.resetLaneArrowFlags: Resetting lane arrows of lane {laneId}.");
                ...
                public bool SetLaneArrowFlags(uint laneId,
                                                     LaneArrows flags,
                                                     bool overrideHighwayArrows = false) {
#if DEBUGFLAGS
            ...
            Log._Debug($"Flags.setLaneArrowFlags({laneId}, {flags}, {overrideHighwayArrows}): setting flags");
                ...
                public void SetHighwayLaneArrowFlags(uint laneId,
                                                            LaneArrows flags,
                                                            bool check = true) {
            ...
            Log._Debug($"Flags.setHighwayLaneArrowFlags: Setting highway arrows of lane {laneId} to {flags}");
                ...
                public bool ToggleLaneArrowFlags(uint laneId,
                                                        bool startNode,
                                                        LaneArrows flags,
                                                        out SetLaneArrow_Result res) {
                    if (!CanHaveLaneArrows(laneId)) {
                ...
                internal bool CanHaveLaneArrows(uint laneId, bool? startNode = null) {
                    if (laneId <= 0) {
                        return false;
                    }
                ...
                public LaneArrows? GetLaneArrowFlags(uint laneId) {
                    return laneArrowFlags[laneId];
                }

                public LaneArrows? GetHighwayLaneArrowFlags(uint laneId) {
                    return highwayLaneArrowFlags[laneId];
                }

                public void RemoveHighwayLaneArrowFlags(uint laneId) {
#if DEBUGFLAGS
            Log._Debug(
                ...
                public bool ApplyLaneArrowFlags(uint laneId, bool check = true) {
#if DEBUGFLAGS
            ...
            Log._Debug($"Flags.applyLaneArrowFlags: Setting lane flags of lane {laneId} to " +
                ...
                public LaneArrows GetFinalLaneArrowFlags(uint laneId, bool check = true) {
                    if (!CanHaveLaneArrows(laneId)) {
#if DEBUGFLAGS
                ...
                public void RemoveLaneArrowFlags(uint laneId) {
                    if (laneId <= 0) {
                        return;
                    }
                ...
                internal void RemoveHighwayLaneArrowFlagsAtSegment(ushort segmentId) {
                    ref NetSegment netSegment = ref segmentId.ToSegment();
                ...
                public void ClearHighwayLaneArrows() {
